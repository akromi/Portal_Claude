...
function sanitizeFileButtons() {
  $('.file-control-container, .container-file-input').each(function () {
    ...
    // IMPORTANT: do NOT remove aria-labelledby here.
    // The relabeler owns aria-labelledby for correct SR announcements.
...
// ---------- File control logger (global) ----------
// NOTE: observeFileControls() is called from page scripts, so the logger must be in global scope.
window.__wetFileCtlLogger = window.__wetFileCtlLogger || (function () {
  const levels = { off: 0, error: 1, warn: 2, info: 3, debug: 4, trace: 5 };
  const getLevel = () => levels[(window.FILE_LOG_LEVEL || 'debug').toLowerCase()] ?? 4;
  const prefix = '[WET4][FileCtl]';
  const fmt = (msg, ...args) => [`${prefix} ${msg}`, ...args];
  return {
    level: () => getLevel(),
    setLevel: (lvl) => { window.FILE_LOG_LEVEL = String(lvl || 'debug'); },
...
// Alias used by existing functions in this file.
// (Keeping this name avoids touching a lot of downstream code.)
var logger = window.__wetFileCtlLogger;
...
window.relabelAllFileUploadControls = function relabelAllFileUploadControls() {
  ...
  // Keep button icon markup; only update the trailing text node.
  function setButtonTrailingText($btn, text) {
    if (!$btn || !$btn.length) return;
    const el = $btn.get(0);
    if (!el) return;

    // Find last text node (nodeType===3)
    let tn = null
    for (let i = el.childNodes.length - 1; i >= 0; i--) {
      const n = el.childNodes[i];
      if (n && n.nodeType === 3) { tn = n; break; }
    }
    if (tn) {
      tn.nodeValue = ' ' + text;
    } else {
  ...
  // Update the choose/change text without destroying the inner <span id="..._choose_text">...
  function setChooseText($chooseBtn, desired, baseId) {
    const $span = $chooseBtn.find('span.file-choose-text, span[id$="_choose_text"]').first();
    if ($span.length) {
      if ($span.text().trim() !== desired) $span.text(desired);
      return $span.attr('id') || '';
    }

    // Fallback: create a choose text span so aria-labelledby can reference it.
    const spanId = baseId ? (baseId + '_choose_text') : ('file_choose_text_' + Math.random().toString(36).slice(2));
    const span = document.createElement('span');
    span.className = 'file-choose-text';
    span.id = spanId;
    span.textContent = desired;

    $chooseBtn.empty().append(span);
    return spanId;
  }

  // Determine server-side file state from PP hidden fields.
  function readServerState($scope, baseId) {
    const $chg = $scope.find('#' + baseId + 'hidden_file_change, #' + baseId + '_hidden_file_change').first();
    const $name = $scope.find('#' + baseId + 'hidden_filename, #' + baseId + '_hidden_filename').first();

    const change = ($chg.val() || '').toString().trim().toLowerCase();
    const filename = ($name.val() || '').toString().trim();
  ...
  $blocks.each(function (idx) {
    ...
    try {
      ...
      const baseId = inputId ? inputId.replace(/_input_file$/, '') : '';
      ...
      if (inputId) {
        ...
        if ($label.length) {
          if (!$label.attr('id')) $label.attr('id', baseId ? (baseId + '_label') : (inputId + '_label'));
          labelId = $label.attr('id') || '';
        }
      }

      // ---- Read PP server hidden fields
      const server = baseId ? readServerState($block.closest('td, .cell, .form-group'), baseId) : { change: '', filename: '' };

      // If the server says the file was deleted, treat as empty and clear stale filename spans.
      const serverDeleted = (server.change === 'delete');
      if (serverDeleted) {
        if ($hiddenSpan.length && ($hiddenSpan.text() || '').trim()) {
          logger.info('block[%d]: server delete detected; clearing stale file_name span for %s', idx, baseId);
          $hiddenSpan.text('');
        }
        if ($textDiv.length && $textDiv.text().trim() !== T.none) $textDiv.text(T.none);
      }

      // ---- compute hasFile (native or server) with delete override
      function computeHasFile() {
        const nativeLen = $input.get(0)?.files?.length || 0;
        if (nativeLen) return true;

        if (serverDeleted) return false;

        const hiddenNameSpan = ($hiddenSpan.text() || '').trim();
        const hiddenVal = (server.filename || '').trim();

        // If server has a filename, that's authoritative.
        if (hiddenVal) return true;

        // Otherwise fall back to filename span ONLY if it's not a "none" placeholder.
        if (hiddenNameSpan && hiddenNameSpan !== T.none) return true;

        return false;
      }

      // ---- apply state (choose/change + delete visibility + labels)
      function applyStateNow() {
        const hasFile = computeHasFile();

        const desiredChoose = hasFile ? T.change : T.choose;
        const chooseTextId = setChooseText($chooseBtn, desiredChoose, baseId);

        // Ensure proper aria-controls
        if (inputId && $chooseBtn.attr('aria-controls') !== inputId) $chooseBtn.attr('aria-controls', inputId);

        // Critical: use aria-labelledby (Option 2)
        // - Normal/tab focus: label + button text
        // - After picker return: button text only (handled via click handler below)
        if (chooseTextId) {
          const full = (labelId ? (labelId + ' ') : '') + chooseTextId;
          // Only set full label if we are not currently in "picker return" mode
          if (!$chooseBtn.data('pickerReturnMode')) {
            if ($chooseBtn.attr('aria-labelledby') !== full) $chooseBtn.attr('aria-labelledby', full);
          }
        }

        // Delete button text + state
        if ($delBtn.length) {
          if (hasFile) $delBtn.show(); else $delBtn.hide();

          // Visible + SR label
          setButtonTrailingText($delBtn, T.delete);
          if ($delBtn.attr('aria-label') !== T.delete) $delBtn.attr('aria-label', T.delete);
          $delBtn.removeAttr('aria-labelledby');

          // Describe filename only (not the field label)
          if ($nameBox.length && !$nameBox.attr('id')) {
            $nameBox.attr('id', baseId ? (baseId + '_filename_status') : ('file_filename_status_' + idx));
          }
        ...
        // Ensure the visible "No file selected" text reflects current language.
        // Do not overwrite when there is a real filename.
        if (!hasFile && $textDiv.length && $textDiv.text().trim() !== T.none) {
          $textDiv.text(T.none);
        }

        logger.debug('block[%d]: applyStateNow -> baseId=%s hasFile=%s serverChange=%s', idx, baseId, hasFile, server.change);
      }

      // Initial state set
      applyStateNow();

      // ---- choose/change semantics (picker return mode)
      const NS = '.fileUxChooseAnnounce';
      $chooseBtn.off(NS);

      // When user activates picker, switch to "button text only" so SR doesn't repeat the label on return.
      $chooseBtn.on('click' + NS, function () {
        const chooseTextId = $chooseBtn.find('span.file-choose-text, span[id$="_choose_text"]').first().attr('id') || '';
        if (chooseTextId) {
          $chooseBtn.data('pickerReturnMode', 1);
          if ($chooseBtn.attr('aria-labelledby') !== chooseTextId) $chooseBtn.attr('aria-labelledby', chooseTextId);
        }
      });

      // On blur, restore full label for next tab-in.
      $chooseBtn.on('focusout' + NS, function () {
        $chooseBtn.data('pickerReturnMode', 0);
        const chooseTextId = $chooseBtn.find('span.file-choose-text, span[id$="_choose_text"]').first().attr('id') || '';
        if (chooseTextId) {
          const full = (labelId ? (labelId + ' ') : '') + chooseTextId;
          if ($chooseBtn.attr('aria-labelledby') !== full) $chooseBtn.attr('aria-labelledby', full);
      ...
      // When file picked/cleared through the real input
      $input.off('change' + NS).on('change' + NS, function () {
        // Keep pickerReturnMode until the user tabs away
        setTimeout(applyStateNow, 0);
      });

      // ---- delete semantics: after server delete, PP leaves stale filename span. Clean it up.
      if ($delBtn.length) {
        const DEL_NS = '.fileUxDeleteFix';
        $delBtn.off(DEL_NS).on('click' + DEL_NS, function () {
          // Do NOT prevent default; let Power Pages delete the file.
          // We just schedule a refresh (and endRequest hook will run again).
          if ($delBtn.data('postDeletePending')) return;
          $delBtn.data('postDeletePending', 1);
          setTimeout(() => {
            try { window.relabelAllFileUploadControls?.(); }
            finally { $delBtn.data('postDeletePending', 0); }
          }, 50);
        });
      }
...
// Single, debounced observer (re-applies after PP redraws) + partial postback hook
function observeFileControls() {
  // Idempotent install: your page scripts may call observeFileControls() more than once.
  if (window.__wetFileUxObserverInstalled) {
    logger.info('observeFileControls: already installed; applying once');
  ...
  window.__wetFileUxObserverInstalled = true;
  ...
  function scheduleApply(reason) {
    ...
    debounceTimer = setTimeout(function () {
      logger.info('observeFileControls: %s -> re-apply', reason);
      applyFilePatches();
    }, DEBOUNCE_MS);
  }
  ...
  // Partial postbacks (UpdatePanel / PageRequestManager)
  if (window.Sys && Sys.WebForms && Sys.WebForms.PageRequestManager) {
    try {
      const prm = Sys.WebForms.PageRequestManager.getInstance();
      if (!window.__wetFileUxEndRequestHooked) {
        window.__wetFileUxEndRequestHooked = true;
        prm.add_endRequest(function () {
          scheduleApply('endRequest');
        });
        logger.info('observeFileControls: Hooked PageRequestManager.endRequest');
      }
    } catch (e) {
      logger.warn('observeFileControls: failed to hook endRequest', e);
    }
  }

  // Expose on window for page scripts
...
(function (global, $) {
  ...
  function patchRadioSummaries($root, radioConfigs) {
    ...
    $.each(radioConfigs, function (_, cfg) {
      ...
      // Focus ring will appear on the legend text (e.g., "Canadian coast guard").
      ...
      $group.find('input[type="radio"]').each(function () {
        ...
        if ($radio.is(':disabled')) {
          // disabled radios are already skipped
          return;
        }
  ...
  function patch(config) {
    ...
    if (config.radioSummaries && config.radioSummaries.length) {
      patchRadioSummaries($root, config.radioSummaries);
    }
...
// ===============================
//
// Makes a PP boolean radio group focusable + read-only on summary
// steps. Tab goes to the checked radio; SR announces the normal
// “Label Yes/No radio button checked 1 of 2” using the existing
// <label for="..."><span class="visually-hidden">Label</span>Yes/No</label>
// (we do not add extra spoken text on the <td> wrapper).
//
...
(function (ns) {
  ...
  /**
   * Make a PP boolean radio group read-only but keyboard focusable on
   * summary steps.
   *
   * @param {Object} cfg
   * @param {string} cfg.baseId  e.g. "ethi_canadiancoastguard"
   * @param {Element|string} [cfg.root] optional root scope (default: document)
   */
  ns.makeRadioCellSummary = function (cfg) {
    try {
      ...
      if (span.dataset) {
        span.dataset.roSummaryApplied = '1';
      }
      ...
      if (cell) {
        ...
        // Keep any existing role="radiogroup" – that is fine.
      }
      ...
      if (checkedIndex < 0) {
        checkedIndex = 0; // fallback to first
      }
